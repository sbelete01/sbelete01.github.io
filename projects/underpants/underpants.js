...
*   _.identity({a: "b"}) === {a: "b"}
*/
// *** important written by me!
// add as property of _ like so
...
_.partition = function(array, action){
    ...
    for (let i = 0; i < array.length; i++){
        if(action(array[i], i, array)){
            trueResult.push(array[i]);
        } else {
            falseResult.push(array[i]);
    }}
    let result = [trueResult, falseResult];
    return result;
};
...
_.map = function(collection, action){
    ...
    if(Array.isArray(collection)){
        ...
        for (let i = 0; i < collection.length; i++){
            // call function on each index
            arrResult.push(action(collection[i], i, collection));
        }
        return arrResult;
    }
    else if (typeof collection === 'object'){
        ...
        for (var key in collection){
            // call function on each property
            objResult.push(action(collection[key], key, collection));
        }
    return objResult;
    }
};
